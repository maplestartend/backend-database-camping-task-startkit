...
SET
  experience_years = 3
WHERE
	id = (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE name = '肌肉棒子'));
...
SET
  experience_years = 5
WHERE
	id = (SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE name = 'Q太郎'));
...
SET
  ...
  status = '課程已取消'
WHERE course_id = (SELECT id FROM "COURSE" WHERE user_id = (SELECT id FROM "USER" WHERE name = '李燕容')) 
...
SELECT 
	...
	COUNT(*) AS total
FROM "COURSE_BOOKING"
WHERE user_id = (SELECT id FROM "USER" WHERE name = '王小明') AND status = '課程已使用'
GROUP BY user_id;
...
from
	"CREDIT_PURCHASE"
inner join "CREDIT_PACKAGE" on
	"CREDIT_PURCHASE".credit_package_id = "CREDIT_PACKAGE".id
where
	extract(month
from
